<template>
  <div class="d-flex flex-column w-100 mr-8">
    ...
    <div class="d-flex flex-row">
      <v-text-field v-model="resonator.data.skill.base_attr" :label="$t('general.base_attr')"></v-text-field>
    </div>
    <div class="d-flex flex-row">
      <v-select v-model="resonator.data.skill.bonus_types" :label="$t('general.skill_bonus_type')" readonly chips
        multiple>
      </v-select>
    </div>
